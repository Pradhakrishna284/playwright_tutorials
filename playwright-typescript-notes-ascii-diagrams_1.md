...
```
┌──────────────────────┐         WebSocket (CDP / custom protocol)      ┌─────────────────────────┐
│   Your Test Script    │ ───────────────────────────────────────────►  │   Browser Process        │
│ (TypeScript, Node.js)  │ ◄─────────────────────────────────────────── │ (Chromium/Firefox/WebKit)│
└──────────────────────┘          events / responses                    └─────────────────────────┘
        │
...
│ (process)    │                      │ (isolated session) │              │ (tab)    │
└─────────────┘                      └──────────────────┘                └─────────┘
                                                                              │
                                                       locator.click()/fill()/etc.
                                                                              ▼
                                                          ┌────────────────────────────┐
                                                          │ Auto-wait engine checks:    │
                                                          │ attached → visible → stable │
                                                          │ → enabled → receives events │
                                                          └────────────────────────────┘
                                                                              │
                                                                              ▼
                                                          Action executed inside real browser
```
...
1. Playwright Test Runner loads playwright.config.ts
   │
   ▼
2. For each project (chromium/firefox/webkit) → launch Browser
   │
   ▼
3. Create new BrowserContext (fresh cookies/storage per test = isolation)
   │
   ▼
4. Create Page → page.goto('https://example.com/login')
   │
   ▼
5. Locator created: page.getByLabel('Username') -- lazy, not resolved yet
   │
   ▼
6. .fill('myuser') called → Playwright:
      a. Resolves locator to actual DOM element
      b. Waits for actionability (visible, enabled, stable)
      c. Sends 'fill' command over CDP
      d. Browser executes & fires events (input/change)
   │
   ▼
7. .click(getByRole('button', {name:'Login'})) → same actionability checks → click dispatched
   │
   ▼
8. expect(page).toHaveURL(/dashboard/) → web-first assertion auto-retries until
   condition true or timeout
   │
   ▼
9. Test passes/fails → trace/screenshot/video captured per config
   │
   ▼
10. Context & Browser closed → next test starts with a brand-new context (isolation)
```
...
action called (e.g. click)
   │
   ▼
Attached? ─ No ─► wait & retry
   │ Yes
   ▼
Visible? ─ No ─► wait & retry
   │ Yes
   ▼
Stable (no animation)? ─ No ─► wait & retry
   │ Yes
   ▼
Receives events (not covered)? ─ No ─► wait & retry
   │ Yes
   ▼
Enabled? ─ No ─► wait & retry
   │ Yes
   ▼
Perform action
```
...
Test file
   │ creates instance
   ▼
LoginPage(page) ──► holds locators + business methods (login(), logout())
   │
   ▼
Test calls loginPage.login(user, pass)
   │
   ▼
Internally: fills username, password, clicks login (all reusable across tests)
   │
   ▼
Test only asserts the outcome — no raw locators in the test file
```
...
test('see inventory', async ({ loggedInPage }) => { ... })
        │
        ▼
Playwright sees fixture name "loggedInPage" requested
        │
        ▼
Runs fixture setup: opens page → logs in
        │
        ▼
use(page) called ──► test body executes with logged-in page
        │
        ▼
Test body finishes
        │
        ▼
Any code after use() in fixture runs as teardown (cleanup)
```
...
page.route('**/api/users', handler) registered
        │
        ▼
Browser makes request to /api/users
        │
        ▼
Playwright intercepts before it leaves/returns
        │
        ▼
   ┌─────────────┬────────────────┬───────────────┐
   │ route.fulfill│ route.continue │ route.abort    │
   │ (mock resp)  │ (modify & send)│ (cancel request)│
   └─────────────┴────────────────┴───────────────┘
        │
        ▼
Browser receives whichever outcome was chosen, renders accordingly
```
...
"setup" project runs ONCE
   │
   ▼
Logs in via UI → captures cookies/localStorage
   │
   ▼
Saves to playwright/.auth/user.json
   │
   ▼
"chromium" project (depends on setup) starts
   │
   ▼
Each new test context loads storageState from user.json
   │
   ▼
Test starts already authenticated — skip login UI every time
```
